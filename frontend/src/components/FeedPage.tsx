...
type DiscoverTab = 'conversations' | 'learning-topics'

interface Props {
  onOpenConversation: (id: string) => void
  onOpenPublicLearningTopic: (id: string) => void
  onGoToLogin: () => void
}
...
function TopicDiscoveryCard({
  ...
  item: PublicLearningTopicDiscoveryItem
  onOpenTopic: () => void
  onReplay: () => void
}) {
  return (
    <div className="w-full text-left bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-gray-400 dark:hover:border-gray-600 rounded-xl overflow-hidden transition-colors group">
      <button
        type="button"
        onClick={onOpenTopic}
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0 flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-gray-300 dark:text-gray-700 text-xs">·</span>
              <span className="text-xs text-gray-500">
                {item.conversation_count} public conversation{item.conversation_count !== 1 ? 's' : ''}
              </span>
            </div>
          </div>
          <svg
      ...
      {item.conversation_count > 0 && (
        <div className="px-5 pb-3 pt-0 flex justify-end border-t border-gray-200/80 dark:border-gray-800/80">
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation()
            ...
            className="text-xs px-3 py-1.5 rounded-lg border border-indigo-300 dark:border-indigo-700 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-800 dark:text-indigo-200 font-medium hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors"
          >
            Replay topic
          </button>
        </div>
...
export function FeedPage({ onOpenConversation, onOpenPublicLearningTopic, onGoToLogin }: Props) {
  const [tab, setTab] = useState<DiscoverTab>('conversations')
  const [feed, setFeed] = useState<FeedResponse | null>(null)
  const [topicsFeed, setTopicsFeed] = useState<PublicLearningTopicDiscoveryResponse | null>(null)
  ...
  useEffect(() => {
    if (tab !== 'learning-topics') return
    setIsLoading(true)
    setError(null)
    const params = new URLSearchParams({ page: String(page), per_page: '20' })
    fetch(getApiUrl(`learning-topics/public?${params.toString()}`))
      .then((res) => {
        if (!res.ok) throw new Error(`Failed to load topics (${res.status})`)
        return res.json() as Promise<PublicLearningTopicDiscoveryResponse>
      })
      .then((data) => {
        setTopicsFeed(data)
        setIsLoading(false)
      ...
      .catch((err: unknown) => {
        setError(err instanceof Error ? err.message : 'Failed to load learning topics.')
        setIsLoading(false)
      })
  }, [tab, page])
  ...
  const activeFeed = tab === 'conversations' ? feed : topicsFeed
  const activePages = activeFeed?.pages ?? 1
  const activePageNum = activeFeed?.page ?? page
    ...
    <div className="flex flex-col min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100">
      ...
      <div className="flex-1 px-4 py-8">
        <div className="max-w-3xl mx-auto flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Browse public conversations and learning topics shared by the community — no sign-in required.
              </p>
            </div>
            <div className="flex p-1 rounded-xl bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 w-full sm:w-auto sm:inline-flex">
              <button
                type="button"
                onClick={() => selectTab('conversations')}
                className={`flex-1 sm:flex-none px-4 py-2 text-xs font-medium rounded-lg transition-colors ${
                  tab === 'conversations'
              ...
              <button
                type="button"
                onClick={() => selectTab('learning-topics')}
                className={`flex-1 sm:flex-none px-4 py-2 text-xs font-medium rounded-lg transition-colors ${
                  tab === 'learning-topics'
                    ? 'bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 shadow-sm border border-gray-200 dark:border-gray-700'
                    : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'
          ...
          {isLoading ? (
          ...
          ) : tab === 'conversations' && (!feed || feed.items.length === 0) ? (
          ...
          ) : tab === 'learning-topics' && (!topicsFeed || topicsFeed.items.length === 0) ? (
            <div className="flex flex-col items-center py-20 gap-3 text-center">
              <span className="text-4xl">📚</span>
              <p className="text-gray-600 dark:text-gray-400 font-medium">No public learning topics yet</p>
              <p className="text-sm text-gray-500 max-w-sm">
                When users set a learning topic to public, it appears here. Each topic lists only conversations they
                have marked public.
              </p>
              <button
                type="button"
                onClick={() => selectTab('conversations')}
                className="mt-2 px-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-800 dark:text-gray-200 text-sm font-medium transition-colors hover:bg-gray-200 dark:hover:bg-gray-700"
              >
            ...
            <>
              <p className="text-xs text-gray-400 dark:text-gray-600 -mt-2">
                {tab === 'conversations' && feed
                  ? `${feed.total} public conversation${feed.total !== 1 ? 's' : ''}${
                      feed.pages > 1 ? ` · Page ${feed.page} of ${feed.pages}` : ''
                    }`
                  : topicsFeed
                    ? `${topicsFeed.total} public learning topic${topicsFeed.total !== 1 ? 's' : ''}${
                        topicsFeed.pages > 1 ? ` · Page ${topicsFeed.page} of ${topicsFeed.pages}` : ''
                      }`
                    : null}
              </p>
              ...
              <div className="flex flex-col gap-2">
                ...
                {tab === 'learning-topics' &&
                  topicsFeed?.items.map((item) => (
                    <TopicDiscoveryCard
                      key={item.id}
                      item={item}
                      onOpenTopic={() => onOpenPublicLearningTopic(item.id)}
                      onReplay={() => setReplayTopicId(item.id)}
                    />
                  ))}
              </div>

              {/* Pagination */}
              {activePages > 1 && activeFeed && (
                <div className="flex items-center justify-center gap-2 pt-4">
                  ...
                  <div className="flex items-center gap-1">
                    {Array.from({ length: activePages }, (_, i) => i + 1)
                      .filter((p) => {
                        const cur = activePageNum
                        return p === 1 || p === activePages || Math.abs(p - cur) <= 2
                      })
                  ...
                  <button
                    onClick={() => goToPage(activePageNum + 1)}
                    disabled={activePageNum >= activePages}
                    className="px-3 py-1.5 text-sm rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    Next →
                  </button>
