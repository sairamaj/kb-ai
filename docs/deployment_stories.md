...
- **Azure OIDC:** The deploy job does **not** use a GitHub **Environment** so the token subject stays **`repo:ORG/REPO:ref:refs/heads/<branch>`**, matching the federated credentials from [docs/developer.md](developer.md) (Z4-04). A job `environment: staging` would emit **`repo:ORG/REPO:environment:staging`** and require an extra federated credential in Entra ID (documented under *Troubleshooting* in the same doc).
