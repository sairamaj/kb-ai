---
name: Cloud deployment requirements
overview: A technology-agnostic list of deployment requirements for the Prompt Knowledge Base app (React frontend, FastAPI backend, PostgreSQL with pgvector), organized into basic deploy, CI/CD, testing, troubleshooting, and other categories.
todos: []
isProject: false
---
...
- **Runbooks and docs**
  - How to run migrations and roll back ([README.MD](README.MD), [docs/developer.md](docs/developer.md)).
...
- **Security**
  ...
  - Align with NFR-SEC-* in [docs/requirements.md](docs/requirements.md) (server-side API keys, JWT/cookie auth, private data access checks).
- **Scalability**
...
- **Documentation**
  - Keep [README.MD](README.MD) and [docs/developer.md](docs/developer.md) updated with production env vars, OAuth setup for production, and any platform-specific steps (without mandating a specific platform).
...
No clarification questions from the codebase; the list is intentionally technology-agnostic. If you later choose a specific cloud or CI system, each requirement can be mapped to concrete tasks (e.g. “run pytest in CI”, “configure health check in platform X”).